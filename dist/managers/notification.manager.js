...
export class NotificationManager {
     ...
     */
    async sendNotification(event, embedData) {
        ...
        try {
            ...
            logError('Failed to deliver notification to primary channel', error, {
                eventId: event.id,
                eventType: event.type,
     ...
     */
    formatPremiumEmbed(data, timestamp) {
     ...
     */
    async handleDeliveryFailure(event, embedData) {
        ...
        if (fallbackChannelId) {
            try {
                ...
                logError('Failed to deliver notification to fallback channel', fallbackError, {
                    eventId: event.id,
                    eventType: event.type,
                    fallbackChannelId,
                });
    ...
    async processQueue() {
        ...
        while (this.notificationQueue.size > 0) {
            ...
            if (notification.lastAttempt) {
                const timeSinceLastAttempt = Date.now() - notification.lastAttempt.getTime();
                if (timeSinceLastAttempt < this.retryDelayMs) {
                    await new Promise((resolve) => setTimeout(resolve, this.retryDelayMs - timeSinceLastAttempt));
                }
            }
            // Attempt delivery
            ...
            try {
                const embed = this.formatPremiumEmbed(notification.embedData, notification.event.timestamp);
                // Get current primary channel ID (supports dynamic updates)
                const primaryChannelId = this.getPrimaryChannelId();
    ...
    async sendNotifications(events) {
        ...
        const sortedEvents = events.sort((a, b) => a.event.timestamp.getTime() - b.event.timestamp.getTime());
    ...
    getQueueStats() {
        const entries = Array.from(this.notificationQueue.values());
        const oldestEntry = entries.sort((a, b) => a.createdAt.getTime() - b.createdAt.getTime())[0];
        return {
            queueSize: this.notificationQueue.size,
     ...
     */
    async sendPunishmentNotification(userId, channelId, punishment, reason, offenseCount) {
        const result = {
            dmSent: false,
            ephemeralSent: false,
            modLogSent: false,
        ...
        try {
            const dmEmbed = new EmbedBuilder()
                ...
                .addFields({ name: 'Reason', value: reason, inline: false }, { name: 'Offense Count', value: offenseCount.toString(), inline: true }, { name: 'Current Punishment', value: punishmentDescription, inline: true }, { name: 'Next Offense', value: punishment.nextPunishment, inline: false })
            ...
            const errorMsg = `Failed to send DM: ${error.message}`;
            result.failures.push(errorMsg);
            logError('Failed to send DM notification', error, { userId });
        }
        ...
        try {
            const ephemeralEmbed = new EmbedBuilder()
                ...
                .addFields({ name: 'Offense Count', value: offenseCount.toString(), inline: true }, { name: 'Next Offense', value: punishment.nextPunishment, inline: false })
                .setColor(this.getPunishmentColor(punishment.type))
                .setTimestamp();
            ...
            const errorMsg = `Failed to send ephemeral message: ${error.message}`;
            result.failures.push(errorMsg);
            logError('Failed to send ephemeral notification', error, { userId, channelId });
        }
        ...
        try {
            ...
            if (modLogChannelId) {
                const modLogEmbed = new EmbedBuilder()
                    ...
                    .addFields({ name: 'User', value: `<@${userId}>`, inline: true }, { name: 'Action', value: punishmentDescription, inline: true }, { name: 'Offense Count', value: offenseCount.toString(), inline: true }, { name: 'Reason', value: reason, inline: false }, { name: 'Channel', value: `<#${channelId}>`, inline: true })
                    .setColor(this.getPunishmentColor(punishment.type))
                    .setTimestamp();
            ...
            const errorMsg = `Failed to send mod-log notification: ${error.message}`;
            result.failures.push(errorMsg);
            logError('Failed to send mod-log notification', error, { userId });
        }
        return result;
    }
    /**
     ...
     */
    formatPunishmentDescription(punishment) {
