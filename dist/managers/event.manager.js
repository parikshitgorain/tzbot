...
export class EventManager {
     ...
     */
    registerHandler(event, handler, priority = 0) {
        const registration = {
            event,
            handler: handler,
            priority,
    ...
    async processEvent(event) {
        ...
        try {
            ...
            for (const registration of handlers) {
                try {
                    await registration.handler(...event.args);
                }
                catch (error) {
                    logError(`Error in event handler for ${String(event.type)}`, error, {
                        eventId: event.id,
