...
export class ChatRainManager {
    ...
    rewardSystem;
    config;
    lastChatRainTime = null;
    constructor(chatActivityRepo, violationRepo, rewardSystem, config) {
     ...
     */
    async executeChatRain(channelId) {
    ...
    async getEligibleRecipients() {
        ...
        const activeChatters = await this.chatActivityRepo.getQualifiedChatters(activeWindow, this.config.minMessages);
     ...
     */
    async getSpamFlaggedUsers(userIds, since) {
        const spamFlaggedUsers = [];
     ...
     */
    selectRecipientCount(availableUsers) {
        const min = 3;
        const max = 10;
     ...
     */
    selectRandomRecipients(users, count) {
    ...
    async recordWinners(winners, timestamp) {
        for (const userId of winners) {
            await this.chatActivityRepo.recordWinner(userId, timestamp, this.config.rewardType, this.config.rewardValue);
        }
    }
    /**
...
//# sourceMappingURL=chat-rain.manager.js.map