...
export class OffenseManager {
    ...
    notificationService;
    constructor(pool, offenseRepository, punishmentCalculator, notificationService) {
        this.pool = pool;
        this.offenseRepository = offenseRepository;
        this.punishmentCalculator = punishmentCalculator;
     ...
     */
    async processOffense(userId, reason, moderatorId, channelId) {
        ...
        try {
            ...
            const punishment = this.punishmentCalculator.calculatePunishment(newOffenseCount, record.current_timeout_duration);
            // Update record
            ...
            try {
                await this.notificationService.sendPunishmentNotification(userId, channelId, punishment, reason, newOffenseCount);
            }
            catch (error) {
                logError('Failed to send punishment notifications', error, {
                    userId,
                    offenseCount: newOffenseCount,
                });
            }
            ...
            logError('Failed to process offense', error, { userId, reason });
            throw error;
        }
        finally {
            client.release();
     ...
     */
    async getOffenseHistory(userId) {
        try {
            return await this.offenseRepository.getOffenseRecord(userId);
        }
        catch (error) {
            logError('Failed to get offense history', error, { userId });
            throw error;
        }
    }
    /**
    ...
    async clearLastOffense(userId) {
        ...
        try {
            ...
            if (record && record.total_offenses > 0) {
                ...
                const punishment = this.punishmentCalculator.calculatePunishment(record.total_offenses, 0);
            ...
            await client.query('ROLLBACK');
            logError('Failed to clear last offense', error, { userId });
            throw error;
        }
        finally {
            client.release();
     ...
     * @param userId - Discord user ID
     */
    async resetAllOffenses(userId) {
        try {
            await this.offenseRepository.resetOffenses(userId);
            logger.info('All offenses reset', { userId });
        }
        catch (error) {
            logError('Failed to reset offenses', error, { userId });
            throw error;
        }
    }
}
//# sourceMappingURL=offense-manager.js.map