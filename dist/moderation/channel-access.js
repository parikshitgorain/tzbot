...
export class ChannelAccessEnforcer {
    ...
    readOnlyChannels;
    moderatorRoleId;
    constructor(discordClient, violationRepo, moderatorRoleId, readOnlyChannelConfigs = []) {
    ...
    async enforceAccess(message) {
        ...
        try {
            ...
            logError('Failed to enforce channel access', error, {
                userId,
                channelId,
                messageId,
            });
     ...
     */
    async sendDMNotification(message, reason) {
        try {
     ...
     */
    hasWhitelistRole(member, whitelistRoleIds) {
     ...
     */
    removeReadOnlyChannel(channelId) {
     ...
     */
    updateWhitelistRoles(channelId, whitelistRoleIds) {
        const config = this.readOnlyChannels.get(channelId);
        if (config) {
     ...
     */
    getWhitelistRoles(channelId) {
        const config = this.readOnlyChannels.get(channelId);
        return config ? config.whitelistRoleIds : [];
    }
}
//# sourceMappingURL=channel-access.js.map