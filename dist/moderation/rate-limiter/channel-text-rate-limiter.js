...
export class ChannelTextRateLimiter {
    ...
    async initialize(config, dependencies) {
        ...
        this.actionHandler = new MessageActionHandler(dependencies.logger, config.warningDeleteDelayMs);
    ...
    async handleMessage(message) {
        ...
        if (violation) {
            ...
            if (redirectChannelId && message.channel instanceof TextChannel) {
                ...
                await this.actionHandler.sendWarning(message.channel, userId, channelName, redirectChannelId);
            }
            // Enter user into violation window
            await this.enforcer.enterViolationWindow(userId, channelId, timestamp, this.config.violationWindowMs);
            return false;
        }
     ...
     */
    async reloadConfig(newRestrictedChannels) {
        if (!this.initialized) {
            throw new Error('ChannelTextRateLimiter not initialized');
        }
...
//# sourceMappingURL=channel-text-rate-limiter.js.map