 ...
 */
async function sendEmbedExample(client, channelId) {
 ...
 */
async function moderationExample(client, guildId, userId) {
    // Warn a user (send DM)
 ...
 */
async function roleManagementExample(client, guildId, userId, roleId) {
...
async function eventDrivenModerationExample() {
    ...
    client.on('messageCreate', async (message) => {
        ...
        if (recentTimestamps.length >= 10) {
            ...
            const oneHour = 60 * 60 * 1000;
            await client.timeoutUser(message.guildId, userId, oneHour, 'Spam detected: 10+ messages in 10 seconds');
            ...
            switch (command) {
                ...
                case 'help': {
                    const helpEmbed = new EmbedBuilder()
                        ...
                        .addFields({ name: '!ping', value: 'Check if bot is responsive' }, { name: '!help', value: 'Show this help message' })
                        .setColor(0x0099ff);
                    await client.sendMessage(message.channelId, {
                        embeds: [helpEmbed],
                    });
