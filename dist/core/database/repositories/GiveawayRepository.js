...
export class GiveawayRepository {
     ...
     */
    async updateStatus(giveawayId, status) {
        const query = `
      UPDATE giveaways
      SET status = $1
      WHERE id = $2
     ...
     */
    async getByChannel(channelId, status) {
        let query = `
      SELECT 
        id, guild_id, title, description, channel_id, message_id,
        required_roles, winner_count, status, ends_at, created_at,
     ...
     */
    async updateWinners(giveawayId, winners) {
        const query = `
      UPDATE giveaways
      SET winners = $1
      WHERE id = $2
...
//# sourceMappingURL=GiveawayRepository.js.map