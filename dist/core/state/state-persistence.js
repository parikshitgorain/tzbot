...
export class StatePersistenceService {
    ...
    async initialize() {
        try {
            ...
            logError('Failed to initialize state persistence service', error, {
                stateDir: this.config.stateDir,
            });
            throw error;
        }
    ...
    async persistState() {
        ...
        try {
            ...
            logError('Failed to persist state', error, {
                stateFile: this.stateFilePath,
            });
        }
        finally {
    ...
    async recoverState() {
        try {
            ...
            if (state.data.chatRain?.lastExecutionTime) {
                state.data.chatRain.lastExecutionTime = new Date(state.data.chatRain.lastExecutionTime);
            }
            ...
            logError('Failed to recover state', error, {
                stateFile: this.stateFilePath,
            });
            // Try to recover from backup
    ...
    async recoverFromBackup() {
        try {
            ...
            logError('Failed to recover from backup', error);
            return null;
        }
    }
    /**
    ...
    async createBackup() {
        try {
            ...
            logError('Failed to create state backup', error);
