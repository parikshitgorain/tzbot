...
export class DataRetentionService {
    ...
    isRunning = false;
    constructor(pool, config = DEFAULT_RETENTION_CONFIG) {
        this.pool = pool;
        this.config = config;
    }
    /**
    ...
    start() {
        ...
        this.runCleanup().catch((error) => {
            logError('Initial cleanup failed', error);
        });
     ...
     */
    async cleanupOldMessageContent() {
        const cutoffDate = new Date();
        cutoffDate.setDate(cutoffDate.getDate() - this.config.messageContentRetentionDays);
        try {
            const result = await this.pool.query(`DELETE FROM message_content 
         WHERE timestamp < $1 
         AND deleted_at IS NULL`, [cutoffDate]);
            ...
            throw new Error(`Failed to cleanup old message content: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
     * Delete chat activity older than retention period
     */
    async cleanupOldChatActivity() {
        const cutoffDate = new Date();
        cutoffDate.setDate(cutoffDate.getDate() - this.config.chatActivityRetentionDays);
        try {
            const result = await this.pool.query(`DELETE FROM chat_activity 
         WHERE timestamp < $1`, [cutoffDate]);
            const deletedCount = result.rowCount || 0;
            if (deletedCount > 0) {
            ...
            throw new Error(`Failed to cleanup old chat activity: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
