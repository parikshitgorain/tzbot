 ...
 */
export class UserLinkingSystem {
    userRepository;
     ...
     */
    async startLinking(discordId) {
        // Check if user already has a pending link
        const existingToken = Array.from(this.pendingLinks.values()).find((link) => link.discordId === discordId);
     ...
     */
    async completeLink(discordId, kickUsername) {
        try {
            ...
            if (existingUser && existingUser.discordId !== discordId) {
                throw new Error(`Kick username ${kickUsername} is already linked to another Discord account`);
            }
            // Link the accounts
     ...
     * Unlink accounts
     */
    async unlinkAccounts(discordId) {
        try {
     ...
     */
    async getKickUsername(discordId) {
        try {
            const user = await this.userRepository.get(discordId);
     ...
     */
    async getDiscordId(kickUsername) {
        try {
     ...
     */
    async processVerificationMessage(message) {
...
//# sourceMappingURL=user-linking.js.map