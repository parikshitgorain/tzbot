...
export class PollingFallbackSystem {
    ...
    enabled;
    currentState = 'webhook';
    pollingTimer = null;
    lastPolledEventTimestamp = null;
    transitionHistory = [];
    isPolling = false;
    constructor(config) {
    ...
    async pollKickAPI() {
        try {
            ...
            const events = await this.kickAPIClient.getLiveEvents(this.channelId, this.lastPolledEventTimestamp || new Date());
            if (events.length > 0) {
            ...
            logError('Error polling Kick API', error, {
                additionalContext: {
                    channelId: this.channelId,
                },
            });
     ...
     */
    async processPolledEvent(event) {
        try {
            ...
            logError('Error processing polled event', error, {
                additionalContext: {
                    eventType: event.type,
                    eventId: event.id,
                },
...
//# sourceMappingURL=polling-fallback.js.map