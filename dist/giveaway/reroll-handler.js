...
export class RerollHandler {
    ...
    constructor(giveawayRepository, winnerStateRepository) {
        this.giveawayRepository = giveawayRepository;
        this.winnerStateRepository = winnerStateRepository;
    }
    /**
     ...
     */
    async getEligibleParticipants(giveawayId) {
        try {
            ...
            const eligibleEntries = giveaway.entries.filter(entry => !excludedUserIds.has(entry.userId));
            // Return array of eligible user IDs
            return eligibleEntries.map(entry => entry.userId);
        }
        catch (error) {
            throw new Error(`Failed to get eligible participants: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
    ...
    async rerollWinner(giveawayId) {
        try {
            ...
            throw new Error(`Failed to reroll winner: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
...
//# sourceMappingURL=reroll-handler.js.map