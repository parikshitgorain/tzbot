 ...
 */
export function createGiveawayCommands(_client, giveawayManager) {
    return [
 ...
 */
function createGiveawayCommand(_client, giveawayManager) {
    ...
    const handler = async (interaction) => {
        ...
        if (subcommand !== 'config') {
            ...
            const canUse = await configManager.canUseGiveawayCommands(interaction.guildId, member);
            if (!canUse) {
                await interaction.reply({
 ...
 */
async function handleCreateGiveaway(interaction, giveawayManager) {
...
async function handleCancelGiveaway(interaction, giveawayManager) {
    ...
    try {
        ...
        logError('Failed to cancel giveaway', error, {
            giveawayId,
            moderator: interaction.user.username,
        });
        const errorMessage = `❌ Failed to cancel giveaway. ${error.message}`;
        if (interaction.deferred) {
            await interaction.editReply({ content: errorMessage });
        }
        else {
 ...
 */
async function handleListGiveaways(interaction, giveawayManager) {
    if (!interaction.guildId) {
        await interaction.reply({
            content: '❌ This command can only be used in a server.',
            ephemeral: true,
    ...
    try {
        ...
        logError('Failed to list giveaways', error, {
            guildId: interaction.guildId,
        });
...
async function handleRerollWinner(interaction, giveawayManager) {
    ...
    try {
        ...
        const errorMessage = `❌ Failed to reroll winner. ${error.message}`;
        if (interaction.deferred) {
            await interaction.editReply({ content: errorMessage });
        }
        else {
 ...
 */
async function handleConfigGiveaway(interaction, giveawayManager) {
    if (!interaction.guildId) {
        await interaction.reply({
            content: '❌ This command can only be used in a server.',
            ephemeral: true,
    ...
    try {
        ...
        if (show || (!addRole && !removeRole && !addUser && !removeUser)) {
            ...
            const userList = config.allowedUsers.length > 0
                ...
                : 'None';
            embed.addFields({ name: 'Allowed Roles', value: roleList, inline: false }, { name: 'Allowed Users', value: userList, inline: false });
            await interaction.editReply({ embeds: [embed] });
            return;
        }
        ...
        await configManager.updateGiveawayPermissions(interaction.guildId, config.allowedRoles, config.allowedUsers);
        ...
        logError('Failed to update giveaway config', error, {
            guildId: interaction.guildId,
        });
...
//# sourceMappingURL=giveaway.commands.js.map