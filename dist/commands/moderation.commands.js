 ...
 */
function initializeOffenseManager(client, channelId) {
    const pool = getPool();
    ...
    return new OffenseManager(pool, offenseRepo, punishmentCalc, notificationManager);
}
/**
 * Create moderation commands
 */
export function createModerationCommands(_client, database, rateLimiter) {
    return [
 ...
 */
function createBanCommand(_client, database) {
    ...
    const handler = async (interaction) => {
        ...
        try {
            ...
            // Send DM to user
            try {
                await user.send(`<@${user.id}> You have been banned from ${interaction.guild?.name}.\nReason: ${reason}`);
            }
            catch (dmError) {
            ...
            logError('Failed to execute ban command', error, {
                guildId,
                userId: user.id,
                reason,
            });
            await interaction.editReply({
                content: `❌ Failed to ban ${user.username}. Error: ${error.message}`,
            });
        }
    };
    return {
 ...
 */
function createTimeoutCommand(_client, database) {
    const builder = new SlashCommandBuilder()
        .setName('timeout')
    ...
    const handler = async (interaction) => {
        ...
        try {
            ...
            // Send DM to user
            try {
                await user.send(`<@${user.id}> You have been timed out in ${interaction.guild?.name} for ${durationMinutes} minutes.\nReason: ${reason}`);
            }
            catch (dmError) {
            ...
            logError('Failed to execute timeout command', error, {
                guildId,
                userId: user.id,
                durationMinutes,
                reason,
            ...
            await interaction.editReply({
                content: `❌ Failed to timeout ${user.username}. Error: ${error.message}`,
            });
        }
    };
    return {
 ...
 */
function createWarnCommand(_client, _database) {
    const builder = new SlashCommandBuilder()
        .setName('warn')
    ...
    const handler = async (interaction) => {
        ...
        try {
            ...
            const punishment = await offenseManager.processOffense(user.id, reason, interaction.user.id, interaction.channelId);
            ...
            logError('Failed to execute warn command', error, {
                guildId,
                userId: user.id,
                reason,
            });
            await interaction.editReply({
                content: `❌ Failed to warn ${user.username}. Error: ${error.message}`,
            });
        }
    };
    return {
 ...
 */
function createKickCommand(_client, database) {
    const builder = new SlashCommandBuilder()
        .setName('kick')
    ...
    const handler = async (interaction) => {
        ...
        try {
            ...
            // Send DM to user
            try {
                await user.send(`<@${user.id}> You have been kicked from ${interaction.guild?.name}.\nReason: ${reason}`);
            }
            catch (dmError) {
            ...
            logError('Failed to execute kick command', error, {
                guildId,
                userId: user.id,
                reason,
            });
            await interaction.editReply({
                content: `❌ Failed to kick ${user.username}. Error: ${error.message}`,
            });
        }
    };
    return {
 ...
 */
function createWarnListCommand(_client, _database) {
    const builder = new SlashCommandBuilder()
        .setName('warnlist')
    ...
    const handler = async (interaction) => {
        ...
        try {
            ...
            const offenseManager = new OffenseManager(pool, offenseRepo, punishmentCalc, notificationManager);
            // Get offense history
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: 'Total Offenses', value: record.total_offenses.toString(), inline: true }, { name: 'Current Timeout', value: `${record.current_timeout_duration}h`, inline: true }, { name: 'Status', value: record.is_banned ? '🚫 Banned' : '✅ Active', inline: true });
            ...
            logError('Failed to execute warnlist command', error, {
                userId: user.id,
            });
            await interaction.editReply({
                content: `❌ Failed to retrieve offense history. Error: ${error.message}`,
            });
        }
    };
    return {
 ...
 */
function createWarnAllCommand(_client, _database) {
    const builder = new SlashCommandBuilder()
        .setName('warnall')
    ...
    const handler = async (interaction) => {
        try {
            ...
            logError('Failed to execute warnall command', error);
            await interaction.editReply({
                content: `❌ Failed to retrieve offense records. Error: ${error.message}`,
            });
        }
    };
    return {
 ...
 */
function createClearWarnCommand(_client, _database) {
    const builder = new SlashCommandBuilder()
        .setName('clearwarn')
    ...
    const handler = async (interaction) => {
        ...
        try {
            ...
            logError('Failed to execute clearwarn command', error, {
                userId: user.id,
            });
            await interaction.editReply({
                content: `❌ Failed to clear warning. Error: ${error.message}`,
            });
        }
    };
    return {
 ...
 */
function createResetOffensesCommand(_client, _database) {
    const builder = new SlashCommandBuilder()
    ...
    const handler = async (interaction) => {
        ...
        try {
            ...
            try {
                await user.send(`<@${user.id}> ✅ **Your offenses have been cleared!**\n\n` +
            ...
            logError('Failed to execute resetoffenses command', error, {
                userId: user.id,
            });
            await interaction.editReply({
                content: `❌ Failed to reset offenses. Error: ${error.message}`,
            });
        }
    };
    return {
 ...
 */
function createModLogCommand(_client, _database) {
    const builder = new SlashCommandBuilder()
        .setName('modlog')
    ...
    const handler = async (interaction) => {
        ...
        try {
            ...
            logError('Failed to execute modlog command', error, {
                userId: user.id,
            });
            await interaction.editReply({
                content: `❌ Failed to retrieve moderation log. Error: ${error.message}`,
            });
        }
    };
    return {
 ...
 */
function createRateLimitAddCommand(_client, _database, rateLimiter) {
    const builder = new SlashCommandBuilder()
    ...
    const handler = async (interaction) => {
        ...
        try {
            ...
            logError('Failed to execute ratelimit-add command', error, {
                restrictedChannelId: restrictedChannel.id,
                redirectChannelId: redirectChannel.id,
            });
            await interaction.editReply({
                content: `❌ Failed to add rate limit. Error: ${error.message}`,
            });
        }
    };
    return {
 ...
 */
function createRateLimitRemoveCommand(_client, _database, rateLimiter) {
    const builder = new SlashCommandBuilder()
    ...
    const handler = async (interaction) => {
        ...
        try {
            ...
            logError('Failed to execute ratelimit-remove command', error, {
                channelId: channel.id,
            });
            await interaction.editReply({
                content: `❌ Failed to remove rate limit. Error: ${error.message}`,
            });
        }
    };
    return {
 ...
 */
function createRateLimitListCommand(_client, _database) {
    const builder = new SlashCommandBuilder()
    ...
    const handler = async (interaction) => {
        try {
            ...
            logError('Failed to execute ratelimit-list command', error);
            await interaction.editReply({
                content: `❌ Failed to list rate limits. Error: ${error.message}`,
            });
        }
    };
    return {
...
//# sourceMappingURL=moderation.commands.js.map