 ...
 */
export function createAnnouncementCommands(discordClient, database, announcementRelay) {
    return [
 ...
 */
function createAnnouncementSetupCommand(database, announcementRelay, discordClient) {
    ...
    return {
        ...
        handler: async (interaction) => {
            try {
                ...
                await interaction.editReply({
                    content: '✅ **Announcement Relay Configured & Active**\n\n' +
 ...
 */
function createAnnouncementAddChannelCommand(database, announcementRelay) {
    const builder = new SlashCommandBuilder()
 ...
 */
function createAnnouncementRemoveChannelCommand(database, announcementRelay) {
    const builder = new SlashCommandBuilder()
 ...
 */
function createAnnouncementStatusCommand(database, announcementRelay) {
    const builder = new SlashCommandBuilder()
        ...
        handler: async (interaction) => {
            try {
                ...
                if (!privateChannelId || publicChannels.length === 0) {
                    await interaction.editReply({
                        content: '⚠️ **Announcement Relay Not Configured**\n\n' +
                ...
                await interaction.editReply({
                    content: '📢 **Announcement Relay Status**\n\n' +
                        `**Status:** ${status}\n` +
 ...
 */
function createAnnouncementToggleCommand(announcementRelay) {
    const builder = new SlashCommandBuilder()
    ...
    return {
        ...
        handler: async (interaction) => {
            try {
                ...
                if (!announcementRelay) {
                    await interaction.editReply({
                        content: '❌ **Announcement Relay Not Available**\n\n' +
...
//# sourceMappingURL=announcement.commands.js.map