...
function createSetupCommand(database, config, announcementRelay) {
    ...
    const handler = async (interaction) => {
        try {
            ...
            logError('Failed to execute setup command', _error);
    ...
    const handler = async (interaction) => {
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                value: config.readOnlyChannels && config.readOnlyChannels.length > 0
            ...
            logError('Failed to execute config command', error);
            await interaction.reply({
 ...
 */
function createUserInfoCommand(database) {
    const builder = new SlashCommandBuilder()
        .setName('userinfo')
    ...
    const handler = async (interaction) => {
        try {
            ...
            logError('Failed to execute userinfo command', error, {
                userId: interaction.user.id,
            });
 ...
 */
function createLinkCommand(database) {
    const builder = new SlashCommandBuilder()
        .setName('link')
    ...
    const handler = async (interaction) => {
        ...
        try {
            ...
            logError('Failed to execute link command', error, {
                additionalContext: {
                    userId: interaction.user.id,
                    kickUsername,
                },
 ...
 */
function createUnlinkCommand(database) {
    const builder = new SlashCommandBuilder()
        .setName('unlink')
    ...
    const handler = async (interaction) => {
        try {
            ...
            logError('Failed to execute unlink command', error, {
                userId: interaction.user.id,
            });
            await interaction.editReply({
 ...
 */
function createCheckLinkCommand(database) {
    const builder = new SlashCommandBuilder()
        .setName('checklink')
    ...
    const handler = async (interaction) => {
        try {
            ...
            logError('Failed to execute checklink command', error, {
                userId: interaction.user.id,
            });
            await interaction.editReply({
 ...
 */
function createDeleteMyDataCommand(database) {
    const builder = new SlashCommandBuilder()
    ...
    const handler = async (interaction) => {
        try {
            ...
            logError('Failed to execute deletemydata command', error, {
                userId: interaction.user.id,
            });
            await interaction.editReply({
