 ...
 */
export type KickWebhookEventType =
  | 'livestream.started'
  | 'livestream.ended'
  | 'livestream.metadata'
  | 'kicks.gifted'
...
export class KickWebhookHandler {
  ...
  private webhookFailureCount: number = 0;
  private lastWebhookSuccess: Date | null = null;
  ...
  verifySignature(payload: string, signature: string): WebhookVerificationResult {
    ...
    try {
      ...
      logError('Error verifying webhook signature', error as Error);
      return {
        valid: false,
  ...
  validatePayload(payload: any): payload is KickWebhookPayload {
    ...
    // Validate event type
    const validEvents: KickWebhookEventType[] = [
      'livestream.started',
      'livestream.ended',
      'livestream.metadata',
      'kicks.gifted',
    ...
    if (!validEvents.includes(payload.event as KickWebhookEventType)) {
  ...
  async handleWebhook(payload: KickWebhookPayload): Promise<void> {
    try {
      ...
      logError('Error handling webhook event', error as Error, {
        additionalContext: {
          event: payload.event,
          timestamp: payload.timestamp,
        },
   ...
   */
  private mapEventType(kickEvent: KickWebhookEventType): EventType {
    const eventMap: Record<KickWebhookEventType, EventType> = {
   ...
   */
  private generateEmbedData(payload: KickWebhookPayload) {
   ...
   */
  private recordWebhookSuccess(): void {
   ...
   */
  private recordWebhookFailure(): void {
